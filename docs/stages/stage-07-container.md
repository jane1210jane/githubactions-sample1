...
- **JSON に金額を文字列で入れた理由** — `etl.py` の `to_json_payload` は
  ...
  JSON の仕様（RFC 8259 第6節）自体は数値の内部表現を規定しておらず、実装が
  受け付ける範囲・精度に制限を設けることを許容しているだけです（同節は
  「IEEE 754 binary64（倍精度）を実装したソフトウェアが広く使われているため、
  それ以上の精度・範囲を期待しなければ相互運用性が得られる」と述べているに
  とどまります）。しかし JavaScript の `JSON.parse` をはじめ、多くの実装が
  実際には倍精度浮動小数点で数値を読むため、`Decimal` の値をそのまま数値として
  書き出すと、読み込み側の実装によっては精度が落ちる可能性があります。金額を
  文字列にしておけば、読み込み側が自分の言語の10進数型（Python の `Decimal` や
...
- **`cache-to` を別ディレクトリに書いて入れ替える理由** — `container.yml` の120〜121行目
  ...
  入れ替えています。`container.yml` 121行目の `mode=max` は、最終ステージが
  直接使う層だけでなく、**中間ステージやベースイメージ由来の層まで含めて**
  キャッシュにエクスポートする指定です（既定の `mode=min` なら最終ステージの
  層だけがエクスポート対象になります）。次の実測のとおり、この構成では
  ベースイメージのレイヤまでキャッシュに含まれる代償が発生しています。
...
- **実測したキャッシュの効き** — 依存を `--extra aws` で固定した後の構成で、
  ...
  いません）、キャッシュのサイズはおよそ217MBでした。**この217MBの大部分は
  `Dockerfile` 自体の3層ではなく、ベースイメージ（`public.ecr.aws/lambda/
  python:3.12`）由来の層（実測では `sha256:9ae56ce...` の層だけで約147MB）です。**
  `container.yml` 121行目の `mode=max` がベースイメージの層までエクスポート対象に
  含めているためで、この構成は「ECR Public から数秒で pull できるベースイメージを、
  GitHub のキャッシュから読み書きし直している」非効率を抱えています。`mode=min`
  にすれば最終ステージの層だけに絞れますが、本教材では `mode=max` の代償を
  実際のログで見せることを優先しています。
...
3. **問3**: `container.yml` の `pull_request` イベントで `push: true` にすると何が起きるか
   予想する。`push:` だけを変えても「GHCR にログインする」ステップの `if:` は
   そのままなので、まずそれだけで何が起きるかを考える。次に、`login` の `if:` も
   併せて外した場合、フォークからの PR と自分のブランチからの PR とで結果が
   分かれる。その差がなぜ生まれるかを説明する。
