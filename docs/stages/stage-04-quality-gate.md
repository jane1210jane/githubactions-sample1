...
以下は実際に行った手順です（このドキュメントでは内部の管理番号ではなく実施内容で
示します）。
...
設定しました（ただしこの `exclude` は現状効いていません。後述します）。
この状態で `uv run mypy src tools` を実行したところ、初回から
...
- **`--cov-fail-under=80` を `pyproject.toml` の `addopts` に置いた理由**:
...
- **`meta` ジョブと `outputs`（37〜53行目）**: `outputs` は、ジョブ間で**文字列**を渡すための
  仕組みです。ファイルを渡したいときは Stage 3 で使った artifact（`upload-artifact` →
  `download-artifact`）を使います。`meta` ジョブは `steps.read.outputs.version`（42行目）
  という**ステップの outputs** を `outputs.version`（同じく42行目）という**ジョブの
  ...
  別の仕組みが存在します。転記ブロックは当時のファイルの内容を保つ約束のため
  修正していませんが、正しい説明は直前の段落のとおりです。
...
- **`outputs` は文字列しか渡せません。** 真偽値のような値も、`"true"` / `"false"` という
  **文字列**として渡ってきます。後続のジョブで `if: needs.foo.outputs.flag` のように
  ...
  必要があります）。
- **matrix ジョブの `outputs` は、どのレグの値が残るかが保証されません。**
  `test` ジョブのように `strategy.matrix` で複数レグに展開されるジョブに `outputs` を
  持たせると、GitHub は各レグの `outputs` を同じジョブ名の下にまとめようとしますが、
  実行順自体がレグごとに変わり得る以上、期待どおりの値が残る保証はない、と捉えて
  おくべきです。本教材が `meta` を `test` とは別の単独ジョブにしたのはこのためです。
  バージョン文字列のように「1つに定まっているべき値」は、matrix ジョブから持ち出さず、
  専用の単独ジョブから出す設計にしています。
- **`$GITHUB_STEP_SUMMARY` は追記（`>>`）で書いてください。** `gate` ジョブの3つの
  ステップのうち、最初の2つはどちらも `$GITHUB_STEP_SUMMARY` に書き込みます
  （161行目・170行目）。どちらも `>>` を使っているため、「結果をステップサマリに書く」で
  書いた表の下に、「PR 向けの案内を出す」の内容が積み重なります。`$GITHUB_STEP_SUMMARY`
  はステップごとに別のファイルを指すため、`>` を使っても**他のステップ**が書いた内容が
  消えることはありません。`>` で消えるのは、あくまで**同じステップ内で先に自分が
  書いた内容**だけです（ジョブ終了時に全ステップぶんのファイルが連結されて1つの
  要約として表示されます）。それでも `>>` を推奨するのは、1つのステップの中で
  複数回 `echo` する場合に、後の行が前の行を消してしまう事故を避けるためです。
...
1. **問1**: テストを1つ削除すると、カバレッジは閾値（80%）を下回るか。まず予想してから、
   実際に1つ削除して確かめる。予想どおりにならなかった場合はなぜかを説明し、
   実際に閾値を割るには何が必要かも調べる。CI が落ちたときのログのどこに理由が
   出ているかも答える。
