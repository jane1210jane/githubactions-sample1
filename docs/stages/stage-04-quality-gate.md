...
- **matrix ジョブの `outputs` は、どのレグの値が残るかが保証されません。**
  ...
  公式ドキュメントはこの挙動を「非決定的」とするに留めており、「必ず最後に完了した
  レグの値になる」といった具体的な規則までは保証していません。実行順自体が
  レグごとに変わり得る以上、期待どおりの値が残る保証はない、と捉えておくべきです。
  本教材が `meta` を `test` とは別の単独ジョブにしたのはこのためです。バージョン文字列
  のように「1つに定まっているべき値」は、matrix ジョブから持ち出さず、専用の単独ジョブ
  から出す設計にしています。
