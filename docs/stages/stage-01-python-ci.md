...
`actions/checkout` は `v7` というメジャー版だけを指す**フロート版タグ**を別途公開しているため、
`ci.yml` では `actions/checkout@v7` と書けます。一方 `astral-sh/setup-uv` は今回調べた時点では
`v9` というフロート版タグをまだ公開しておらず（`v8` も同様）、存在するのは `v9.0.0` という
完全版タグのみでした。存在しないタグを指定すると `uses:` の解決自体に失敗しステップが
即座に落ちるため、`ci.yml` では `astral-sh/setup-uv@v9.0.0` と完全版で指定しています。
（フロート版タグは公開側の運用次第で増減するため、実装時は Step 1 のコマンドで必ず現物を
確認してください。）
...
- **`uses:`**（18行目 `uses: actions/checkout@v7`、21行目 `uses: astral-sh/setup-uv@v9.0.0`）:
  ...
  ランナーは本当に空のままでした。
- **`with:`**（22〜23行目）: アクションに渡す引数です。`astral-sh/setup-uv@v9.0.0` に対して
  `python-version: "3.12"` を渡し、インストールする Python のバージョンを指定しています。
- **バージョン指定 `@v7` / `@v9.0.0`**（18行目・21行目）: `uses:` には必ずバージョンを
  つけます。省略はできません。ここでは `actions/checkout` はメジャー版のフロート版タグ
  `@v7` を、`astral-sh/setup-uv` は Step 1 で確認した完全版タグ `@v9.0.0` を使っています
  （理由は Step 1 の解説を参照）。コミット SHA まで固定するようなより厳密な指定方法は
  Stage 6 で扱います。
