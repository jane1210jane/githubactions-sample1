...
| 8 | AWS へ実デプロイ | OIDC による一時認証（長期アクセスキーの廃止）、ECR への push、**Lambda（コンテナイメージ）へのデプロイ**、`environments` と承認フロー、staging → production、エイリアスによるロールバック、artifact でのジョブ間受け渡し（Stage 0 で予告した回収） |
...
### 転記ブロックの宣言と検証の強化（フェーズ2完了後の改訂、Stage 6 で実施）

フェーズ2の最終レビューで、現在の検査ツールに2つの穴が残っていることが判明した。

1. **引用先ファイルを判別できない。** ツールは文書内の全転記ブロックの行番号を1つの集合にまとめるため、
   `action.yml` の12行目を指したつもりの引用が `ci.yml` の12行目で満たされてしまう。
   Stage 5 の解説は転記ブロックを3つ持つが、正しさは人手の監査でしか担保できていない。
2. **転記ブロック自体の陳腐化を検出できない。** ツールは「引用先の番号が存在するか」しか見ておらず、
   転記が `stage-NN` 時点のファイルと一致しているかは検証していない。引用がすべて解決したまま
   転記だけが古くなる状態を、機械では検出できない。

対処として、各転記ブロックの直前に出所を宣言させる。

```
<!-- transcript: .github/workflows/ci.yml @ stage-05 -->
```

検査ツールはこの宣言を読み、(a) `git show <tag>:<path>` の内容と転記ブロックを行単位で突き合わせ、
(b) 各引用を、本文が名指ししているファイルの宣言ブロックに限定して照合する。
これにより人手監査への依存が機械的な保証に置き換わる。

### フェーズ末の整合スイープ（フェーズ2完了後の追加）

各フェーズの最終タスクとして、**そのフェーズの変更によって過去の文書が偽になっていないか**を
洗う専用タスクを置く。フェーズ2では、README のパス記述・トラブル索引のシェル助言・
Stage 5 の重複箇所の説明という3件の矛盾が、いずれも「各タスクが自分の触るファイルしか見ない」
ために誰にも気づかれないまま残った。個々のタスクのレビューでは構造的に検出できない種類の欠陥である。

### 計画由来の事実誤認への備え（フェーズ2完了後の追記）

フェーズ2の最終レビューで、実装・レビュー2回を素通りした事実誤認が2件見つかった
（`GITHUB_STEP_SUMMARY` の上書き挙動、`outputs` がジョブ間で値を渡す「唯一の」手段だという説明）。
どちらも**計画文書の時点で**書かれた記述がそのまま解説に転記され、レビューが
「実装は計画どおりか」だけを確認していたために誤りごと通過していた。ここから得られる
教訓は、**計画に書かれた GitHub Actions の仕様に関する事実は、転記してよい文ではなく、
実装時に検証すべき主張として扱う**、ということ。次フェーズ以降、計画に仕様の断定
（「〜は唯一の方法」「〜は必ず…になる」のような言い切り）が含まれる場合は、実装時に
一次情報（公式ドキュメントまたは実測）で裏を取ってから解説に反映する。
...
**Stage 8 のデプロイ先は Lambda（コンテナイメージ）とする。** Stage 7 で作ったコンテナをそのまま
デプロイでき、題材である Python ETL に最も近く、待機コストが実質ゼロで学習者が放置しても課金されない。
OIDC・ECR・関数更新・エイリアスによるロールバックという実務の要素が一通り揃う点も選定理由である。

**ECS Fargate は実装せず、「選ぶ場合の差分」として解説にまとめる。** 本格的なコンテナ運用に一番近いが、
VPC・ALB・タスク定義といった AWS 側の前提知識が多く、待機中も課金される。Actions の学習より
AWS の学習に時間を取られるため、実装対象からは外す。ただし業務で選択肢になりうるため、
Stage 8 の解説に「ワークフローのどこが変わるか」「必要になる AWS 資源」「コストモデル」
「どちらを選ぶべきか」を整理した節を置く。
